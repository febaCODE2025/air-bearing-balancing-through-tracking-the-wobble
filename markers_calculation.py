...
def run_tracking():
    ...
    # Save the final path image before closing
    os.makedirs(os.path.dirname(FINAL_IMAGE_FILE), exist_ok=True)
    
    # Draw the coordinate system on the final background frame
    bg_h, bg_w, _ = background_frame.shape
    cv2.line(background_frame, (bg_w // 2, 0), (bg_w // 2, bg_h), (255, 0, 0), 2)
    cv2.line(background_frame, (0, bg_h // 2), (bg_w, bg_h // 2), (255, 0, 0), 2)
    cv2.circle(background_frame, (bg_w // 2, bg_h // 2), 5, (0, 255, 0), -1)

    cv2.imwrite(FINAL_IMAGE_FILE, background_frame)
    print(f"Final path plot saved to {FINAL_IMAGE_FILE}")
...
def analyze_data(data):
    ...
    # The variables to be optimized are the center (xc, yc) and radius (r).
    ...
    # The correction vector points from the center of rotation (0,0) to the center of the path.
