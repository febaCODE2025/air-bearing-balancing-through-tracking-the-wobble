...
def run_tracking():
    ...
    # Separate path lists for pixel and mm coordinates
    marker_paths_pixel = {}
    marker_paths_mm = {}
    ...
    # Separate persistent backgrounds for pixel and mm plots
    ...
    while (time.time() - start_time) < RUN_SECONDS:
        ...
        # --- Pixel-based path plotting and data collection ---
        if ids is not None:
            ...
            # --- Millimeter-based data collection ---
            if len(REF_COORDS_PIXELS) == 4 and CENTER_ID in ids_flat:
                ...
                # --- Millimeter-based path plotting ---
                if CENTER_ID not in marker_paths_mm:
                    marker_paths_mm[CENTER_ID] = []
                mm_to_px_scale = background_frame_mm.shape[0] / (R_REF * 4) # Adjust for better plotting
        ...
        # Combine pixel background with live frame for display
        ...
        cv2.line(display_frame, (w // 2, 0), (w // 2, h), (255, 0, 0), 2)
        cv2.line(display_frame, (0, h // 2), (w, h // 2), (255, 0, 0), 2)
...
def analyze_data(data, background_mm):
    ...
    # Load the pixel plot to add a coordinate system and the correction vector.
    ...
    # --- Plotting on the pixel plot image ---
    ...
    # Use the same homography calculation from run_tracking to get the pixel equivalent of the analysis data.
    # We need to re-calculate homography here as `M` is not in this function's scope.
    if len(REF_COORDS_PIXELS) == 4:
        src_pts = np.float32([REF_COORDS_PIXELS[1], REF_COORDS_PIXELS[2], REF_COORDS_PIXELS[3], REF_COORDS_PIXELS[4]]).reshape(-1, 1, 2)
        dst_pts = np.float32([REF_COORDS_REAL[1], REF_COORDS_REAL[2], REF_COORDS_REAL[3], REF_COORDS_REAL[4]]).reshape(-1, 1, 2)
        M, _ = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 5.0)

        # Convert the fitted center from mm to pixels
        fitted_center_mm = np.array([[x_path_center, y_path_center]])
        fitted_center_px = cv2.perspectiveTransform(fitted_center_mm.reshape(-1, 1, 2), np.linalg.inv(M))
        fitted_center_px = fitted_center_px[0][0]
        
        # Draw the correction vector on the pixel plot
        correction_vector_end_mm = np.array([[-x_path_center, -y_path_center]])
        correction_vector_end_px = cv2.perspectiveTransform(correction_vector_end_mm.reshape(-1, 1, 2), np.linalg.inv(M))
        correction_vector_end_px = correction_vector_end_px[0][0]
        
        # Plot the correction vector as an arrow on the pixel plot.
        cv2.arrowedLine(pixel_plot_image, tuple(fitted_center_px.astype(int)), tuple(correction_vector_end_px.astype(int)), (255, 255, 255), 5)
    ...
    center_px_x = int(w / 2 + x_path_center * mm_to_px_scale)
    center_px_y = int(h / 2 - y_path_center * mm_to_px_scale)
    cv2.circle(final_image, (center_px_x, center_px_y), 5, (255, 0, 0), -1) # Blue dot for center
    
    radius_px = int(radius * mm_to_px_scale)
    cv2.circle(final_image, (center_px_x, center_px_y), radius_px, (255, 0, 255), 2) # Pink circle
    
    correction_vector_end_x = int(w / 2 - x_path_center * mm_to_px_scale)
    correction_vector_end_y = int(h / 2 + y_path_center * mm_to_px_scale)
    cv2.arrowedLine(final_image, (center_px_x, center_px_y), (correction_vector_end_x, correction_vector_end_y), (255, 255, 255), 5) # White arrow
    
    cv2.line(final_image, (w // 2, 0), (w // 2, h), (0, 0, 255), 2) # Red Y-axis
    cv2.line(final_image, (0, h // 2), (w, h // 2), (0, 0, 255), 2) # Red X-axis
    cv2.circle(final_image, (w // 2, h // 2), 5, (0, 255, 0), -1) # Green circle at origin
